...
pub fn install_bundle(
  ...
  if !Path::new(&dir_bundles).exists() {
    ...
    unwrap_ok_or!(fs::create_dir(dir_bundles), e, {
      return format_error("Failed to create bundles directory", e);
    });
  }
  logger.emit_progress(1);

  logger.emit("Fetching version list...");
  let versions = unwrap_ok_or!(git::fetch_versions_for_url(&parsed_url.bundle_url), e, {
    return format_error("Failed to get version list", e);
  });
    ...
    Ok(repo) => {
      ...
      if versions.len() > 1 {
        ...
        unwrap_ok_or!(
          ...
          e,
          { return format_error("Failed to check out latest version", e) }
        )
      }
    }
    Err(e) => {
      return format_error(
        &format!("Failed to clone bundle '{}'", parsed_url.bundle_name),
        e,
      )
    }
  }
...
pub fn fetch_bundle_versions(
  ...
  let repo = unwrap_ok_or!(try_open_repository(path), e, {
    return format_error("Failed to open repository", e);
  });
  ...
  if repo.is_none() {
    ...
    let remote = unwrap_ok_or!(git::get_remote(&unwrapped_repo), e, {
      return format_error("Failed to get remote info", e);
    });
...
pub fn set_bundle_version(
  ...
  match Repository::open(path) {
    Ok(repo) => {
      logger.emit_progress(2);
      let mut remote = unwrap_ok_or!(git::get_remote(&repo), e, {
        return format_error("Failed to get remote repository", e);
      });
      unwrap_ok_or!(
        remote.fetch(
          &[""],
        ...
        e,
        return format_error("Failed to fetch version data", e)
      );
      unwrap_ok_or!(git::checkout_version(&repo, version.clone()), e, {
        return format_error(&format!("Failed to checkout version {}", version), e);
      });
      logger.emit_progress(3);
    }
    Err(e) => {
      return format_error(
        &format!("Failed to open git repository for bundle '{}'", bundle_name),
        e,
      )
    }
  }
    ...
    Ok(repo) => {
      if repo.is_some() {
        match get_tag_name_at_head(&repo.unwrap()) {
          ...
          Err(e) => {
            return format_error(
              &format!("Failed to get version for bundle '{}'", bundle_name),
              e,
            )
          }
        }
